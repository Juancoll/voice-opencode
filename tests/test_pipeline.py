...
def test_stop_dictation_injects_via_paste_when_configured(isolated_lock, monkeypatch):
    ...
    # Must NOT go through press_key (wtype) — that's the bug we're fixing.
    press_key.assert_not_called()
...
def test_stop_dictation_paste_falls_back_to_press_key_when_no_ydotool(
    ...
    """If ydotool isn't installed, paste falls through to desktop.press_key
    (wtype). Documented limitation — works on apps that respect
